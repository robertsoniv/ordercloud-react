...
function getPropertyLabel(property: string) {
  const split = property.split(".").length
    ? last(property.split("."))
    : property;
  if (!split) return;
    ...
    if (schemaShape[propName]) {
      schemaShape[propName] =
        (requiredProps && requiredProps.includes(propName)) || target.required
          ? schemaShape[propName].required(
              `${getPropertyLabel(propName)} is required`,
            )
          : target?.type === "object"
...
function typeSpecificTransform(type: string, propName: string) {
  return (_: any, val: any) =>
    val === null || hasValueOfType(type, val, propName) ? val : undefined;
}
...
export function shallowNestedProperties(obj: any) {
  ...
  for (let key in obj) {
    if (obj[key]?.hasOwnProperty("allOf")) {
      ...
      shallowObj[key].properties = shallowNestedProperties(
        shallowObj[key].properties,
      );
    } else {
