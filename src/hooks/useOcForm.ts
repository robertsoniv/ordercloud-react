...
import {
  ...
  shallowNestedProperties,
  shapeXpSchema,
} from "../formSchemaGenerator.utils";
...
export const useOcForm = (
  ...
  const { saveOperation, createOperation, assignmentSaveOperation } =
    useOperations(resource, operationInclusion) as any;
  const { xpSchemas } = useOrderCloudContext();

  const isNew = useMemo(
    () =>
      !initialValues?.body ||
      (!initialValues?.body && !Object.keys(initialValues.body).length),
    [initialValues?.body],
  );

  const requestSchema = useMemo(() => {
    const schema = isAssignment
      ? assignmentSaveOperation?.requestBody?.content["application/json"].schema
      : isNew
    ...
    assignmentSaveOperation?.requestBody?.content,
    createOperation?.requestBody?.content,
    isAssignment,
    isNew,
    saveOperation?.requestBody?.content,
  ]);
  ...
  const xpSchema = useMemo(() => {
    if (xpSchemas?.properties && xpSchemas.properties[resource]) {
      ...
      initialValues?.body?.xp &&
      Object.keys(initialValues?.body?.xp).length
    ) {
  ...
  const resourceSchema = useMemo(() => {
    ...
    if (nativeProperties && nativeProperties.xp && xpSchema) {
      ...
      if (
        (xpSchemas?.properties?.[resource] as OpenAPIV3.SchemaObject)?.required
      ) {
        (
          xpSchemas?.properties?.[resource] as OpenAPIV3.SchemaObject
        )?.required?.forEach((propKey: string) => {
  ...
  const getDefaultSchemaValues = useCallback(
    (schema: OpenAPIV3.SchemaObject, parentPath?: string) => {
      ...
      Object.entries(schema).forEach(
        ([key, value]: [string, OpenAPIV3.SchemaObject]) => {
          ...
          obj[key] = normalizeExampleValue(
            getExampleValue(requestSchema?.allOf[0].example, path),
          );
        },
      );
      return obj;
  ...
  const formSchema = useMemo(() => {
    ...
    const generatedSchema = generateFormSchema(
      resourceSchema,
      requestSchema?.required,
    );
    ...
    if (operationParameters) {
      generatedParamSchema = generateFormSchema(
        operationParameters?.schema,
