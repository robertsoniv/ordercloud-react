...
const useColumns = (
  ...
  } = useOperations(resourceId);

  const sortable = useMemo(() => {
    const params = operation?.parameters as OpenAPIV3.ParameterObject[];
    const schema = params?.find(
      (p: OpenAPIV3.ParameterObject) => p?.name === "sortBy",
  ...
  const properties = useMemo(() => {
    ...
    const schemaItemArray = schema?.properties
      ?.Items as OpenAPIV3.ArraySchemaObject;
    ...
    (obj: unknown, accessor?: string) => {
      ...
      Object.entries(obj).forEach(([key, value]) => {
        ...
        if (type === "object") {
          ...
          const header =
            accessorString.includes("xp.") &&
            accessorString.split(".").length === 2
              ? accessorString
              : key;
          cols.push(
  ...
  const assignmentProperties = useMemo(() => {
    const response = assignmentListOperation?.responses[
      "200"
