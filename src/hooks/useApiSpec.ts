...
const psuedoResources: IPsuedoResource[] = [
  {
    ...
    getOperationId: (resourceId, o) => {
      return (
        resourceId + "." + o?.operationId?.split(".")[1].split("Option")[0]
      );
    },
  },
  {
  ...
  },
  {
    ...
    getOperationId: (resourceId, o) => {
      return (
        resourceId + "." + o?.operationId?.split(".")[1].split("Shipment")[0]
      );
    },
  },
  {
    ...
    getOperationId: (resourceId, o) => {
      return (
        resourceId + "." + o?.operationId?.split(".")[1].split("Promotion")[0]
      );
    },
  },
  {
    ...
    getOperationId: (resourceId, o) => {
      return (
        resourceId +
        "." +
        o?.operationId?.split(".")[1].split("EligibleApprover")[0]
      );
    },
  },
  {
    ...
    getOperationId: (resourceId, o) => {
      return (
        resourceId + "." + o?.operationId?.split(".")[1].split("Approval")[0]
      );
    },
  },
];

const buildOperationsForResource = (
  spec: OpenAPIV3.Document,
  psuedoResource: IPsuedoResource,
) => {
  return flatten(
    psuedoResource.paths.map((path) => {
      ...
      return values(
        mapValues(ops, (o: any, verb: string) => {
          return {
            ...o,
            operationId: psuedoResource.getOperationId(
              Case.pascal(psuedoResource.name),
              o,
            ),
            verb,
            path,
...
const useApiSpec = () => {
  ...
  const validateSpecVersion = useCallback(
    async (url: string, version: string) => {
      if (specEnv?.data?.BuildNumber && specEnv.data.BuildNumber !== version) {
        console.log(
          `Current spec (v${version}) is outdated, updating to ${specEnv.data.BuildNumber}`,
        );
        retrieveSpec(url);
      }
    },
  ...
  const result = useMemo(() => {
    ...
    const schemas = spec
      ? (spec!.components!.schemas as Record<string, OpenAPIV3.SchemaObject>)
